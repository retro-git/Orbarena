...
void
FollowCamera::updateFollowPoint()
{
  ...
  if (followPointRadius <= 0.1) {
    ...
    if (dist > 0.1) {
      lerpAlpha =
        min(pow(0.1, ofGetLastFrameTime()), (double)(followPointRadius / dist));
    }
...
bool
FollowCamera::playerControl()
{
  if (myApp->inputMouseVertical != 0 || myApp->inputMouseHorizontal != 0) {
    this->lookAngles +=
      rotationMaxAccel * ofGetLastFrameTime() *
      (glm::vec2(-myApp->inputMouseVertical, myApp->inputMouseHorizontal) *
       sensitivity);
    if (lookAngles.y < 0)
      lookAngles.y += 360;
...
FollowCamera::autoControl()
{
  ...
  float targetRotDiffAbs =
    abs(angleDelta(lookAngles.y, dirToAngle(glm::normalize(diff))));
  if (targetRotDiffAbs < alignmentAngleRange) {
    ...
    rotationMaxAccelThisFrame *=
      (180.f - targetRotDiffAbs) / alignmentAngleRange;
  }
